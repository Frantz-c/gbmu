...
#define ENABLE_EXTERNAL_RAM_MBC1()	\
	...
	{\
		...
		g_memmap.extern_ram = g_memmap.extern_ram_banks[g_memmap.cur_extern_ram];\
	}\
	else\
	{\
		g_memmap.cart_reg[0] = 0;\
		g_memmap.extern_ram = g_memmap.complete_block + 0x2000;\
	}



cycle_count_t __attribute__((noinline))	write_cartridge_register(uint32_t i, uint8_t value, cycle_count_t cycles)
{

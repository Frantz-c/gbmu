...
/*
	dprintf(log_file, "\nPC = 0x%x, ADDR = 0x%lx\n"
					...
					"stack = %hx %hx %hx\n"
					"cc4e = 0x%hhx 0x%hhx 0x%hhx 0x%hhx 0x%hhx 0x%hhx 0x%hhx\n\n\t",
					...
					*(uint16_t*)(GET_REAL_ADDR(regs->reg_sp)), *(uint16_t*)(GET_REAL_ADDR(regs->reg_sp + 2)), *(uint16_t*)(GET_REAL_ADDR(regs->reg_sp + 4)),
					*(uint8_t*)GET_REAL_ADDR(0xcc4e), *(uint8_t*)GET_REAL_ADDR(0xcc4f), *(uint8_t*)GET_REAL_ADDR(0xcc50), *(uint8_t*)GET_REAL_ADDR(0xcc51),
					*(uint8_t*)GET_REAL_ADDR(0xcc52), *(uint8_t*)GET_REAL_ADDR(0xcc53), *(uint8_t*)GET_REAL_ADDR(0xcc54)
			);
...
plog("ld_hl_sp_imm8\n");
	ADD_PC(2);
	regs->reg_f = 0;
	imm_16 = (uint16_t)(regs->reg_sp + imm_8);
	if (imm_16 < regs->reg_sp)
		regs->reg_f |= FLAG_CY;
	if ((imm_16 & 0xFFFu) < (regs->reg_sp & 0xFFFu))
		regs->reg_f |= FLAG_H;
